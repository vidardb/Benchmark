...
class TBAMBenchmarkScenario : public DBBenchmarkScenario {
  public:
...
void TBAMBenchmarkScenario::BenchInsertScenario(void* args) {
...
void TBAMBenchmarkScenario::BenchGetScenario(GetType type) {
...
bool TBAMBenchmarkScenario::PrepareBenchmarkData() {
    work T(*C);
    tablewriter W(T, string(kTableName));
    ifstream in(string(getenv(kDataSource)));
...
vector<string> TBAMBenchmarkScenario::EncodeTuple(const string& line) {
    vector<string> row;
    row.reserve(15);
